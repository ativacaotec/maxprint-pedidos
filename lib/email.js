...
const NOME_REMETENTE = process.env.EMAIL_NOME || 'Catálogos Ativação';
...
function montarHtml(pedido, urlPainel, nomeMarca) {
  ...
  <div style="max-width:640px;margin:0 auto;background:#fff">
    <div style="background:#000;padding:18px 24px">
      <div style="color:#fff;font-size:18px;font-weight:bold">Pedido novo${nomeMarca ? ` · ${escapar(nomeMarca)}` : ''} nº ${pedido.numero}</div>
    ...
    <div style="padding:14px 24px;background:#f4f4f5;font-size:11px;color:#8D8E8E">
      Ativação Group${nomeMarca ? ` · representação ${escapar(nomeMarca)}` : ''}
    </div>
  </div></body></html>`;
}
...
async function enviarAvisoPedido(pedido, destinatarios, urlPainel, nomeMarca = '') {
  if (!configurado()) {
  ...
  const assunto = `Pedido ${pedido.numero}${nomeMarca ? ` · ${nomeMarca}` : ''} · ${pedido.razaoSocial} · ${dinheiro(pedido.total)}`;
  const html = montarHtml(pedido, urlPainel, nomeMarca);
