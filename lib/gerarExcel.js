 ...
 * @param {object} pedido
 * @param {object} [opcoes]
 *   nomeMarca — nome da marca no cabeçalho. Estava chumbado "MAXPRINT" aqui, e
 *   a planilha de um pedido Samsonite saía dizendo Maxprint. É essa planilha
 *   que serve para digitar no portal da indústria: cabeçalho errado é pedido
 *   digitado no portal errado.
 */
async function gerarExcel(pedido, opcoes = {}) {
  ...
  ws.getCell('B1').value = `PEDIDO Nº ${pedido.numero}  ·  ATIVAÇÃO GROUP${nomeMarca ? '  ·  ' + nomeMarca.toUpperCase() : ''}`;
  ...
  for (const it of ordenados) {
    ...
    // A coluna de entrega carrega também a unidade de venda e a tarja: é esta
    // planilha que vai para o portal da indústria, e "24" sem unidade pode
    // virar 24 peças num item vendido em embalagem de 12 (= 288 peças).
    r.getCell(7).value = [
      it.natureza === 'programado'
