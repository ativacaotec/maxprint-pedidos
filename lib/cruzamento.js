...
function agruparCores(produtos) {
  ...
  for (const p of produtos) {
    ...
    const conjunto = irmaos.length ? irmaos : [p];
    conjunto.forEach((c) => usados.add(c.codigo));

    // Cor com preço diferente vira card próprio: não é a mesma compra.
    const porPreco = new Map();
    for (const c of conjunto) {
      const chave = Number(c.precoBase || 0).toFixed(4);
      if (!porPreco.has(chave)) porPreco.set(chave, []);
      porPreco.get(chave).push(c);
    }

    for (const cores of porPreco.values()) {
      cards.push({
