...
async function importarCatalogoYins(caminhoPdf, opcoes = {}) {
  ...
  try {
    ...
    // O nome da tabela vem do cabeçalho do próprio catálogo, não do nome do
    // arquivo: "TABELA VOLTA ÀS AULAS (3)_compressed.pdf" virava "VOLTA ÃS
    // AULAS (3)" no menu do cliente, com o acento quebrado e a cópia do
    // navegador junto.
    const nomeTabela = titulo || tituloDoCatalogo(paginas, path.basename(caminhoPdf));
        ...
        for (const it of itens) {
          ...
          for (const sku of it.cores) {
            ...
            produtos.push({
              ...
              catalogo: nomeTabela,
              segmento: segmentoDaPagina(secoes, n) || nomeTabela,
              pagina: n,
              cor: limpo(sku.cor),
...
/**
 * O nome da tabela, tirado do próprio catálogo.
 *
 * Antes eu usava o nome do arquivo, e o resultado apareceu no menu do cliente
 * como "VOLTA ÃS AULAS (3)" e "YINS PAPER (19) (1)": o "(3)" é a cópia baixada
 * do navegador e o "Ã" é o acento do nome do arquivo lido como latin-1. O
 * catálogo, por outro lado, escreve o próprio nome no topo de toda página
 * ("TABELA VOLTA ÁS AULAS", "TABELA YIN'S KIDS") — é a fonte certa, e não muda
 * quando o arquivo é renomeado.
 */
function tituloDoCatalogo(paginas, nomeArquivo) {
  // Pula a capa: lá o nome aparece desenhado em duas camadas e sai duplicado
  // ("Tabela Chris Chris Helena Helena"). Da segunda página em diante o
  // cabeçalho é uma linha só, igual em todas.
  const amostra = (paginas || []).slice(1, 10);
  const linhas = [];
  for (const pg of amostra) {
    const faixa = pg.palavras.filter((w) => w.y < Math.max(45, pg.altura * 0.035)
      && w.x < pg.largura * 0.6);
    if (!faixa.length) continue;
    // Só a linha mais alta da faixa. Em algumas páginas o nome do primeiro
    // produto começa logo abaixo do cabeçalho e entrava junto.
    const topo = Math.min(...faixa.map((w) => w.y));
    // Palavra repetida em cima da própria palavra: o cabeçalho da Mochilas é
    // desenhado em duas camadas, 2 pt de diferença, para dar contorno. Lido
    // sem cuidado vira "TABELA TABELA MOCHILAS MOCHILAS".
    const ordenadas = faixa.filter((w) => w.y - topo < 5).sort((a, b) => a.x - b.x);
    const unicas = ordenadas.filter((w, i) => {
      const antes = ordenadas[i - 1];
      return !(antes && antes.txt === w.txt && Math.abs(antes.x - w.x) < 12);
    });
    const linha = unicas.map((w) => w.txt).join(' ').replace(/\s+/g, ' ').trim();
    if (linha) linhas.push(linha);
  }

  const conta = new Map();
  for (const l of linhas) conta.set(l, (conta.get(l) || 0) + 1);
  let melhor = '';
  let vezes = 0;
  for (const [texto, n] of conta) if (n > vezes) { melhor = texto; vezes = n; }
  // Cabeçalho é o que se REPETE. Aparecendo uma vez só, é texto de produto.
  if (vezes < 2) melhor = '';

  let nome = melhor
    .replace(/\bSum[áa]rio\b/gi, '')
    .replace(/^TABELA\s+(DE\s+)?/i, '')
    .replace(/\s+/g, ' ')
    .trim();

  if (!nome || nome.length < 3) {
    // Sem cabeçalho legível, sobra o nome do arquivo — mas com o acento
    // remontado e sem o "(3)" que o navegador acrescenta em cópia repetida.
    let cru = String(nomeArquivo || '');
    try {
      const remontado = Buffer.from(cru, 'latin1').toString('utf8');
      if (!/\uFFFD/.test(remontado)) cru = remontado;
    } catch (_) { /* fica como veio */ }
    nome = cru
      .replace(/\.pdf$/i, '')
      .replace(/\((\d+)\)/g, '')
      .replace(/[_-]+/g, ' ')
      ...
      .replace(/^\s*TABELA\s+(DE\s+)?/i, '')
      .replace(/\s+/g, ' ')
      .trim();
  }
  if (!nome) return 'Catálogo';

  // Título em caixa alta no PDF vira Capitalizado na tela do cliente.
  const minusculas = new Set(['de', 'da', 'do', 'das', 'dos', 'e', 'em', 'a', 'o', 'às', 'ás', 'as']);
  return nome.toLocaleLowerCase('pt-BR').split(' ').map((p, i) => {
    if (i && minusculas.has(p)) return p;
    return p.charAt(0).toLocaleUpperCase('pt-BR') + p.slice(1);
  }).join(' ');
}

module.exports.tituloDoCatalogo = tituloDoCatalogo;
