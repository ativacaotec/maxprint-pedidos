...
const { ehCabecalhoDeTabela, ehSobraDeDescricao } = require('./nomeDeProduto');
...
function lerCard(grupo, pagina) {
  ...
  // Segundo corte: cabeçalho de tabela e sobra de descrição não são nome.
  //
  // Nas páginas de etiqueta o cabeçalho "CÓD. MODELO FORM. DA ETIQ. QTDE. POR
  // EMB." vem no mesmo corpo de letra do nome e passava batido — 37 produtos
  // foram para o ar com esse texto no lugar do nome, e o cliente lia isso no
  // catálogo. O filtro de cima não pegava porque ele exige "COD:" com
  // dois-pontos e sem acento.
  const limpas = candidatasNome.filter((l) => !ehCabecalhoDeTabela(l.texto) && !ehSobraDeDescricao(l.texto));

  // Se o corte não deixou nada, vale o conjunto antigo: melhor um nome feio do
  // que perder o produto na importação.
  const usadas = limpas.length ? limpas : candidatasNome;

  const maiorCorpo = usadas.length ? Math.max(...usadas.map((l) => l.tamanho)) : 0;
  const nome = usadas
