...
function gerarPdf(pedido, opcoes = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // O PDF trazia "Maxprint" chumbado no cabeçalho e no rodapé. Com o sistema
    // atendendo mais de uma marca, o pedido de Samsonite ia para o cliente com
    // o nome da outra fábrica. Passa a sair a marca do próprio pedido.
    // O mapa aqui embaixo é só plano B: quem chama passa `nomeMarca` da marca
    // cadastrada. Sem isso, a Yin's — a terceira marca — saía sem nome nenhum
    // no cabeçalho e no rodapé, e qualquer marca nova nasceria com o mesmo
    // defeito sem ninguém tocar em código.
    const nomeMarca = opcoes.nomeMarca
      || ({ maxprint: 'Maxprint', samsonite: 'Samsonite', yins: "Yin's" }[pedido.marcaSlug] || '');
    ...
    doc.font('Helvetica').fontSize(9).fillColor(CINZA)
      .text(`${nomeMarca ? nomeMarca + ' · ' : ''}emitido em ${new Date(pedido.createdAt || Date.now()).toLocaleString('pt-BR')}`,
    ...
    for (const it of itens) {
      ...
      doc.font('Helvetica').fontSize(7.5).fillColor(CINZA)
        .text(
          ...
          // Quem digita o pedido na fábrica lê esta linha. "24" sozinho não
          // diz se são 24 peças ou 24 embalagens de 12 — e a diferença é de
          // 264 peças. A tarja REDUZIDO avisa que pode não vir tudo.
          (it.unidadeVenda ? `  ·  vendido por ${it.unidadeVenda}` : '') +
          (String(it.situacaoEstoque || '').toUpperCase() === 'REDUZIDO' ? '  ·  ESTOQUE REDUZIDO' : ''),
    ...
    for (let i = 0; i < paginas.count; i++) {
      ...
      // O rodapé é escrito ABAIXO da margem inferior. Com a margem valendo, o
      // pdfkit entendia que o texto não cabia e criava uma página nova só para
      // ele — todo PDF de pedido saía com uma folha em branco no fim, e o
      // rodapé dizia "página 1 de 1" numa peça de duas páginas. Zerar a margem
      // durante a escrita mantém o rodapé no pé da própria página.
      const margemBaixo = doc.page.margins.bottom;
      doc.page.margins.bottom = 0;
      doc.font('Helvetica').fontSize(7.5).fillColor(CINZA).text(
        `Ativação Group${nomeMarca ? ` · representação ${nomeMarca}` : ''} · pedido ${pedido.numero} · página ${i + 1} de ${paginas.count}`,
        MARGEM, doc.page.height - 28, { width: larguraUtil, align: 'center', lineBreak: false }
      );
